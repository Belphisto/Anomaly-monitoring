...
def _generate_mdissid_series(node_seed: int):
    rng = np.random.default_rng(30_000 + node_seed)
    x = np.linspace(0.0, 4.5 * np.pi, SERIES_LENGTH)
    cpu = 1.1 + 0.08 * np.sin(x) + rng.normal(0.0, 0.01, SERIES_LENGTH)
    voltage = 10.1 + 0.12 * np.cos(x / 2.5) + rng.normal(0.0, 0.015, SERIES_LENGTH)

    end = ANOMALY_START + ANOMALY_LENGTH
    cpu[ANOMALY_START:end] += 2.8 + 0.15 * rng.normal(size=ANOMALY_LENGTH)
    voltage[ANOMALY_START:end] += 6.8 + 0.22 * rng.normal(size=ANOMALY_LENGTH)
    for idx in (ANOMALY_START + 5, ANOMALY_START + 14):
        cpu[idx] += 1.2
        voltage[idx] += 2.0
...
def test_mdissid_load_sla_multi_node_replay(tmp_path, pytestconfig, node_count):
    prepared_dir = tmp_path / 'prepared'
    prepared_dir.mkdir()
    for i in range(node_count):
        cpu_values, voltage_values = _generate_mdissid_series(i)

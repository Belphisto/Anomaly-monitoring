...
def test_lstm_sla_wall_clock_on_synthetic_stream(tmp_path, pytestconfig):
    ...
    record_sla_result(pytestconfig, 'LSTM', replay_seconds=result['elapsed'])

    assert result["elapsed"] < TOTAL_WALL_CLOCK_SLA_SECONDS
    assert result["peak_score"] == pytest.approx(29.0)


def test_lstm_sla_detects_shifted_window_with_limited_latency(tmp_path, pytestconfig):
    prepared_dir = tmp_path / "prepared"
    prepared_dir.mkdir()
    write_node_csv(
        prepared_dir / "node001.csv",
    ...
    record_sla_result(pytestconfig, 'LSTM', latency_steps=detection_latency)

...
class DummyGauge:
    def __init__(self,name,documentation,labelnames=None):
        self.name=name; self.documentation=documentation; self.labelnames=tuple(labelnames or []); self._children={}
...
class _StubDAMPResult:
    def __init__(self, score, position): self.score=score; self.position=position
class StubMultidimDAMPStreamDetector:
    def __init__(self, window_size, start_index, init_backward_factor):
        self.window_size=window_size; self.start_index=start_index; self.init_backward_factor=init_backward_factor; self.samples_seen=0
    def update(self, cpu, voltage):
        self.samples_seen += 1
        if self.samples_seen < self.window_size: return None
...
class StubLSTMReplayScorer:
    def __init__(self, model): self.model=model; self.window=[]; self.ready=False
    def update(self, cpu, voltage):
        self.window.append((cpu, voltage))
        if len(self.window) < self.model.window_size:
            self.ready=False; return None
        if len(self.window) > self.model.window_size: self.window.pop(0)
        self.ready=True
        cpu_vals=np.array([x[0] for x in self.window], dtype=float)
        volt_vals=np.array([x[1] for x in self.window], dtype=float)
        c=float(np.mean(cpu_vals)); v=float(np.mean(volt_vals))
